...
df_new=df[:10]          #erste 10 Zeilen liegen vor mit Datum und Schlusskurs
...
df_new['Adj Close']= pd.Series(list(range(len(df))))                    #Das war glaube noch nicht so richtig, nochmal ransetzen und überprüfen!
df_new['Adj Close'].plot(x='Adj Close',y='P/L Total')                   #Hier auch noch nicht richtig denke ich!

df_new.plot(x='Adj Close',y='P/L Total')                                #Das sollte nun das richtige sein denke ich 
#wenn man jetzt mehr als die verwendeten 10 werte nutzt könnt eman noch den Durchschnitt des P/L Total bilden wan dann ja ungefähr 0 sein sollte ca oder ggf sogar dann Profit hängt halt von der betrachteten Underlying Periode ab!